...
struct OccupancySensorAccessory : Service::OccupancySensor {
    ...
    OccupancySensorAccessory(uint8_t pin, bool inverted = true, uint16_t cooldownSec = 0)
        : Service::OccupancySensor() {
        // OccupancyDetected: 0 = Occupancy not detected, 1 = Occupancy detected
    ...
    void setOccupancyDetected(bool rawPinState) {
        ...
        if (occupancyDetected->getVal() != newVal) {
            occupancyDetected->setVal(newVal);
        }
    }

    // Get current occupancy state
    bool isOccupied() const {
        return occupancyDetected->getVal() == 1;
    }
