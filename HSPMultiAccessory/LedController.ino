...
void LedController::Update() {
  if (type == RGBLED || type == RGBWLED)
    ...
    float kelvin = 1000000.0 / ColorTemperature;

    // Compute PWM duty cycles for warm and cool channels
    float cool = mapKelvinToCoolRatio(kelvin);
    float warm = 1.0 - cool;

    // Apply brightness
    float level = Brightness / 100.0;
    float coolPWM = level * cool * 255.0;
    float warmPWM = level * warm * 255.0;

    // Set PWM outputs
    analogWrite(W_pin, (int)coolPWM);
    analogWrite(R_pin, (int)warmPWM);
...
float mapKelvinToCoolRatio(float kelvin) {
    // Map between 2700K (warm) and 6500K (cool)
    if (kelvin <= 2700) return 0.0;
    if (kelvin >= 6500) return 1.0;
    return (kelvin - 2700.0) / (6500.0 - 2700.0);
}
