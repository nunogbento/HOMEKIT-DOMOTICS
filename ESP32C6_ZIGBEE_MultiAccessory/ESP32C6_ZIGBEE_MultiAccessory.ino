...
#define IR_SELFTEST_MS 300000UL                    // bench diagnostic window after boot

/* IR carrier is generated by the RMT peripheral, not by the library.
 * IRremoteESP8266 bit-bangs the carrier with digitalWrite + delayMicroseconds,
 * and on this chip the loop bookkeeping (counter, bounds test, micros()) all
 * lands in the off phase: measured on the scope, a frame asking for 38 kHz came
 * out at 30.6 kHz (98 carrier cycles inside the 3200 us LG2 header mark). Its
 * calibrate() cannot correct that — it assumes the error scales with the period
 * while the real overhead is a fixed ~11 us adder. 19 % low puts the carrier
 * well down the bandpass skirt of a 38 kHz receiver, which is the long-standing
 * "IR only works pointed straight at the AC" complaint on these boards.
 * RMT clocks the carrier in hardware, so it is exact and the Zigbee task cannot
 * jitter it. The library is kept purely to BUILD the frame (all the LG mode /
 * temp / fan / swing / checksum logic) — we take getRaw() and clock it out. */
#define IR_CARRIER_HZ   38000
/* rmtSetCarrier() wants a FRACTION, not a percentage: its `if (duty_percent > 1)`
 * guard silently forces 0.5. 0.33 is what sendLG2() asked for; 0.5 is the knob
 * for more range once 38 kHz alone has been judged against the real split. */
...
/* Clock one 28-bit LG2 frame out through RMT. 30 symbols: header, 28 data bits,
 * footer mark + tail space. Blocks for the ~56 ms the frame lasts. */
static bool g_irRmtReady = false;

static bool irRmtBegin() {
  if (!rmtInit(IR_LED_PIN, RMT_TX_MODE, RMT_MEM_NUM_BLOCKS_1, IR_RMT_TICK_HZ)) {
    Serial.println("IR: rmtInit failed");
    return false;
  }
  /* carrier_level must be LOW: the HAL assigns it straight to
   * rmt_carrier_config_t.flags.polarity_active_low, so HIGH puts the carrier in
   * the spaces and leaves the marks DC-on (measured). */
  if (!rmtSetCarrier(IR_LED_PIN, true, LOW, IR_CARRIER_HZ, IR_CARRIER_DUTY)) {
    Serial.println("IR: rmtSetCarrier failed");
    return false;
  }
  rmtSetEOT(IR_LED_PIN, LOW);   // park the LED off between frames
  Serial.printf("IR: RMT ready on IO%d, %d Hz carrier at %.0f%% duty\n",
                IR_LED_PIN, IR_CARRIER_HZ, IR_CARRIER_DUTY * 100.0f);
  return true;
}
...
void setup() {
  ...
  if (g_acEnabled) {
    /* The library builds the frame; RMT transmits it. Deliberately NO
     * calibrate() and NO IRsend::begin() — nothing must touch the pad but RMT. */
    g_ir = new IRLgAc(IR_LED_PIN);
    g_ir->setModel(lg_ac_remote_model_t::AKB75215403);   // same remote as the HomeKit firmware
    g_irRmtReady = irRmtBegin();
...
void loop() {
  ...
  /* BENCH DIAGNOSTIC: for the first IR_SELFTEST_MS after boot, send one frame
   * every 3 s. Deterministic timing means the scope does not have to be armed in
   * step with a Z2M write. Drop this block once the AC is verified. */
  if (g_irRmtReady && g_ir && now < IR_SELFTEST_MS) {
    static unsigned long nextSelfTest = 2000;
    if ((long)(now - nextSelfTest) >= 0) {
      nextSelfTest = now + 3000;
      g_ir->on();
      g_ir->setMode(kLgAcCool);
      g_ir->setTemp(20);
      g_ir->setFan(kLgAcFanAuto);
      const uint32_t code = g_ir->getRaw();
      irRmtSendLg2(code);
      Serial.printf("IRTEST frame sent via RMT (cool 20 C fan=auto) raw=0x%07lX\n",
                    (unsigned long)code);
    }
  }
