#ifndef CARBON_MONOXIDE_SENSOR_H
#define CARBON_MONOXIDE_SENSOR_H

#include "HomeSpan.h"

struct CarbonMonoxideSensorAccessory : Service::CarbonMonoxideSensor {

    SpanCharacteristic *coDetected;
    uint8_t _pin;
    bool _inverted;
    ...
    CarbonMonoxideSensorAccessory(uint8_t pin, bool inverted = true, uint16_t cooldownSec = 0)
        : Service::CarbonMonoxideSensor() {
        // CarbonMonoxideDetected: 0 = CO levels normal, 1 = CO levels abnormal
        coDetected = new Characteristic::CarbonMonoxideDetected(0);
        _pin = pin;
        _inverted = inverted;
    ...
    void setCODetected(bool rawPinState) {
        ...
        if (coDetected->getVal() != newVal) {
            coDetected->setVal(newVal);
        }
    }

    // Get current detection state
    bool isCODetected() const {
        return coDetected->getVal() == 1;
    }
...
#endif // CARBON_MONOXIDE_SENSOR_H
